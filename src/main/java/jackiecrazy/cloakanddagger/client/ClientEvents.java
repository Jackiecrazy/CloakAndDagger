...
public class ClientEvents {
        ...
        for (String s : pos) {
            try {
                ...
                if (GeneralConfig.debug)
                    CloakAndDagger.LOGGER.warn("improperly formatted custom rotation definition " + s + "!");
            }
        }
    }
    ...
    public static void dodge(InputUpdateEvent e) {
        ...
        final IVision itsc = CombatData.getCap(mc.player);
...
//            }
    ...
    public static void sweepSwingOff(PlayerInteractEvent.RightClickEmpty e) {
        if (!rightClick && GeneralConfig.dual && e.getHand() == Hand.OFF_HAND && ((CombatUtils.isWeapon(e.getItemStack()) || (e.getItemStack().isEmpty() && CombatData.getCap(e.getPlayer()).isCombatMode())))) {
            rightClick = true;
