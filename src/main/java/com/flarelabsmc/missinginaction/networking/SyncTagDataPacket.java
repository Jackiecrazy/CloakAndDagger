...
public class SyncTagDataPacket {
    ...
    private static final FriendlyByteBuf.Writer<CombatUtils.StabInfo> info = (f, info) -> info.write(f);

    private static final FriendlyByteBuf.Reader<TagKey<Item>> ritem = f -> ItemTags.create(new ResourceLocation(MissingInAction.MODID, f.readResourceLocation().getPath()));;
    private static final FriendlyByteBuf.Reader<CombatUtils.StabInfo> rinfo = CombatUtils.StabInfo::read;
    private final Map<TagKey<Item>, CombatUtils.StabInfo> map;

    public SyncTagDataPacket(Map<TagKey<Item>, CombatUtils.StabInfo> map) {
        this.map = map;
    }
